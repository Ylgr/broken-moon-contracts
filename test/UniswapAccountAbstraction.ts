...
describe('UniswapAccountAbstraction', () => {
    ...
    it("should swap USDT to BIC in normal case", async () => {
        ...
        await usdtContract.connect(wallet1).approve(routerAddress as any, ethers.parseEther("100") as any);
        // swap USDT to BIC
        ...
        const tx = await routerContract.connect(wallet1).swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline);
        await tx.wait();
